...
typedef enum {
  AUTO_LOCAL_INFILE_OFF=0,  /* heuristics is disabled, use CLIENT_LOCAL_FILES */
  AUTO_LOCAL_INFILE_ON=1,   /* heuristics is enabled, not sending files */
  AUTO_LOCAL_INFILE_SERVING /* heuristics is enabled, ready to serve a file */
...
mysql_init(MYSQL *mysql)
{
  ...
  */
  ...
  mysql->auto_local_infile= ENABLED_LOCAL_INFILE == LOCAL_INFILE_MODE_AUTO
                            ? AUTO_LOCAL_INFILE_ON : AUTO_LOCAL_INFILE_OFF;
#endif
...
static my_bool cli_read_query_result(MYSQL *mysql)
{
  ...
  ulong length;
#ifdef MYSQL_CLIENT
  my_bool can_local_infile= mysql->auto_local_infile != AUTO_LOCAL_INFILE_ON;
#endif
  DBUG_ENTER("cli_read_query_result");

  if (mysql->auto_local_infile == AUTO_LOCAL_INFILE_SERVING)
    mysql->auto_local_infile= AUTO_LOCAL_INFILE_ON;
...
mysql_send_query(MYSQL* mysql, const char* query, ulong length)
{
      ...
      (*query == 'l' || *query == 'L'))
  {
    if (strncasecmp(query, STRING_WITH_LEN("load")) == 0)
      mysql->auto_local_infile= AUTO_LOCAL_INFILE_SERVING;
  }
